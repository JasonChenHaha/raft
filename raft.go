...
type Config struct {
	...
	// 查阅https://blog.csdn.net/Cassie_zkq/article/details/115724570
...
type raft struct {
	...
	// value.
	// 在raft状态机成为leader的初始化过程中，还设置了pendingConfIndex，主要用于在恢复整个集群的时候，
	// 忽略所有集群变更信息，只有当所有在pendingConfIndex之前的日志被apply后，才可以进行集群变更操作。
	// 那么这里就是leader所有的日志被apply之后，才能进行集群变更操作。
...
func stepLeader(r *raft, m pb.Message) error {
	...
	case pb.MsgBeat:
		r.bcastHeartbeat()
		return nil
	case pb.MsgCheckQuorum: // leader检查是否还有法定peer活跃, 如果没有, 降为follower
	...
	case pb.MsgAppResp:
		...
		if m.Reject {
			...
			if m.LogTerm > 0 {
				// 全部说的是跳跃探测方式
				// 这段说的是, 如果follower的reject包是{term:2,index:6}, leader应该探测{term:1,index:1}
				// 而不是探测{term:5,index:6}
				...
				//
				// 这段说的是如何优化follower来优化线性探测
				// 其实就是反过来的跳跃
				// 当leader发来探测包{term:3,index:8}, follower应该返回reject包{term:3,index:3}
				// 来帮助leader跳跃
				...
				//    log.
				// 查找rejectHint的上一个term的index作为下一次的探测位
...
func (r *raft) handleAppendEntries(m pb.Message) {
	...
	if mlastIndex, ok := r.raftLog.maybeAppend(m.Index, m.LogTerm, m.Commit, m.Entries...); ok {
		// 当m.term == raft.term时, 告知对方拼接后的位置
		r.send(pb.Message{To: m.From, Type: pb.MsgAppResp, Index: mlastIndex})
	} else {
		// 当m.term != raft.term时进入, 告知对方冲突开始时的位置
...
// 根据已经committed的len(data)来增大uncommitted的size
...
// 根据已经committed的len(data)来缩小uncommitted的size
