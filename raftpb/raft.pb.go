...
type SnapshotMetadata struct {
	...
	Index     uint64    `protobuf:"varint,2,opt,name=index" json:"index"`	// snapshot中最后一条已提交索引的index和term
...
// 计算x实际占用多少个字节
// 这里是根据protobuf的varint的二进制结构来计算的
// varint每个byte首位是状态位, 1表示下一次byte也是数字的一部分
// 0表示结束, 因此每个byte有效数据位只有7位
